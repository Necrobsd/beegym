...
def send_message(text_message_id=None, photo_message_id=None):
    try:
        bot = DjangoTelegramBot.get_bot()
        if photo_message_id:
            message = PhotoMessages.objects.get(photo_message_id)
            ...
            message.save()
        if text_message_id:
            message = TextMessages.objects.get(text_message_id)
